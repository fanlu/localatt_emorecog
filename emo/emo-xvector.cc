...
class BatchedXvectorComputer {
 public:
  ...
  void ComputeOneChunk(CuMatrix<BaseFloat> &cu_output);
  ...
  void AddChunkStreamToBatch(XvectorTask *task,
                       OnlineFeatureInterface *input,
                       int32 chunk_start);
...
void BatchedXvectorComputer::AddChunkStreamToBatch(
    XvectorTask *task,
    OnlineFeatureInterface* input,
    int32 chunk_start) {
  // int32 n = position_in_batch_++;
  ...
  int32 n = 0;
  int32 T = opts_.chunk_size,
      num_input_frames = input->NumFramesReady();
  KALDI_ASSERT(input_feats_.NumRows() == T * opts_.batch_size);
  if (input->Dim() != feature_dim_) {
    KALDI_ERR << "Feature dimension mismatch: neural net expected "
              << feature_dim_ << ", got " << input->Dim();
  }
  for (int32 t = 0; t < T; t++) {
    SubVector<BaseFloat> dest(input_feats_, t * opts_.batch_size + n);
    int32 src_t = t + chunk_start;
...
void BatchedXvectorComputer::ComputeOneChunk(CuMatrix<BaseFloat> &cu_output) {
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage: emo-xvector [options] <raw-nnet-in> "
    ...
    std::string use_gpu = "no", nnet_config;
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      // KALDI_LOG << "sample frequency:" << wave_data.SampFreq();
      Matrix<BaseFloat> features, pitch_features;
      ...
      try {
        ...
        int32 samp_offset = 0, chunk_offset=0;
        // create zero length task
        BatchedXvectorComputer::XvectorTask *task = computer.CreateTask(utt, 0);
        while(samp_offset < data.Dim()) {
          ...
          // KALDI_LOG << "dim: " << feature_pipeline.InputFeature()->Dim()
          //           << " numframes: " << feature_pipeline.InputFeature()->NumFramesReady();
          samp_offset += num_samp;
          if (samp_offset == data.Dim()) {
            // no more input. flush out last frames
            feature_pipeline.InputFinished();
            KALDI_LOG << " numframes: " << feature_pipeline.InputFeature()->NumFramesReady();
            int32 chunk_start = feature_pipeline.InputFeature()->NumFramesReady() - chunk_offset*opts.chunk_size;
            computer.AddChunkStreamToBatch(task, feature_pipeline.InputFeature(), chunk_start);
            task->num_chunks++;
            KALDI_LOG << "task: " << task->num_chunks << " chunk_start:" << chunk_start;
            // computer.ComputeOneBatch();
            CuMatrix<BaseFloat> cu_output;
            // computer.ComputeOneBatch();
            computer.ComputeOneChunk(cu_output);
            // Matrix<BaseFloat> output(cu_output);
            KALDI_LOG << cu_output.Row(0);
          }else{
            // add opt.chunk_size frame to task
            if (feature_pipeline.InputFeature()->NumFramesReady() > (chunk_offset+1) * opts.chunk_size) {
              computer.AddChunkStreamToBatch(task, feature_pipeline.InputFeature(), chunk_offset*opts.chunk_size);
              task->num_chunks++;
              KALDI_LOG << "task: " << task->num_chunks << " chunk_start:" << chunk_offset*opts.chunk_size;
              CuMatrix<BaseFloat> cu_output;
              // computer.ComputeOneBatch();
              computer.ComputeOneChunk(cu_output);
              // Matrix<BaseFloat> output(cu_output);
          ...
          // SlidingWindowCmn(cmn_opts, feat, &cmvn_feat);
          // Vector<BaseFloat> voiced(feat.NumRows());
          // ComputeVadEnergy(opts, feat, &voiced);
        ...
        // std::vector<int32> chunk_starts;
        // int32 num_frames = feature_pipeline.InputFeature()->NumFramesReady();
        // computer.SplitUtteranceIntoChunks(num_frames, &chunk_starts);

        // mfcc.ComputeFeatures(waveform, wave_data.SampFreq(), vtln_warp_local,
        //                     &features);
        // ComputeKaldiPitch(pitch_opts, waveform, &pitch_features);
        // Matrix<BaseFloat> processed_feats(pitch_features);
        // ProcessPitch(process_opts, pitch_features, &processed_feats);
        frame_count += feature_pipeline.InputFeature()->NumFramesReady();
      } catch (...) {
